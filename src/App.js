...
function NonCombatStatsStep({ data, setData, onNext, onBack }) {
  ...
  const getAvailableSubDomainPoints = () => {
  ...
  const updateDomain = (domainName, newValue) => {
    ...
    if (Object.values(newDomains).reduce((total, val) => total + getStatCost(val), 0) <= 180) {
  ...
  const currentTotal = getSubDomainPointsSpent(subDomains);
  const costBefore = getStatCost(currentValue);
  const costAfter = getStatCost(clampedValue);
